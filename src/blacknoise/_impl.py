...
class Blacknoise:
    def __init__(self, application, *, immutable_file_test=lambda *a: False):
        self._files = {}
        self._prefixes = ()
    ...
    def add(self, path, prefix):
        ...
        for base, _dirs, files in os.walk(path):
            self._files |= {os.path.join(prefix, file): os.path.join(base, file) for file in files}
    ...
    async def __call__(self, scope, receive, send):
        ...
        if file := self._files.get(path):
            headers = {"cache-control": FOREVER if self._immutable_file_test(path) else A_LITTE_WHILE}
